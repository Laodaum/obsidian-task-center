...
- 字符串数组 `["a","b"]` 或逗号分隔串 `"a,b"` → 包含组 **AND**（任务须**同时含**全部标签），无排除。
- 对象 `{ "values": ["a","b"], "mode": "and" | "or" }` → 显式选包含组模式。**OR** = 任务含其中**任一**标签即可（四象限/上下文分组常用）。
- 对象再带 `"exclude": ["c"]` → **排除组**：任务只要带其中任一标签就被过滤掉。匹配 =（包含组为空，或按 `mode` 命中）**且**（排除组一个都不命中）。空包含 + 有排除 = 只做排除。

都会自动补 `#` 前缀、按小写去重；同一标签同时出现在两组时以包含为准。归一化后：纯 AND 包含组且无排除 → 收敛回裸数组，否则保留 `{ values, mode, exclude? }` 对象形态。（图形面板：每个标签行点击循环「不管 → 含 → 排除」；包含标签 ≥2 时出现「全部 / 任一」切换。）（US-109d3）
