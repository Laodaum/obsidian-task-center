...
没有「整个 Tab 的基础集合」。每个 list / grid / week / month area，以及 list 内的每个 section，都带**自己的 `when`**，它就是这块区域的全部过滤。GUI 的过滤面板编辑的就是这个 `when`，与你在 JSON 里手写完全等价。
...
- 字符串数组 `["a","b"]` 或逗号分隔串 `"a,b"` → **AND**（任务须**同时含**全部标签）。
- 对象 `{ "values": ["a","b"], "mode": "and" | "or" }` → 显式选模式。**OR** = 任务含其中**任一**标签即可（四象限/上下文分组常用）。

都会自动补 `#` 前缀、按小写去重。归一化后：AND 收敛回裸数组，OR 保留 `{ values, mode: "or" }` 对象形态（图形过滤面板里选了 ≥2 个标签时会出现「全部 / 任一」切换）。
...
| `sections` | 把一个 list 内部按各自的 `when` 再切成若干带标题的分组（见下）。 |
...
「今日」视图就是**一个** list（area 级 `when` 取 `status: todo`），内部用 `sections` 分成逾期 / 今天 / 未排期三组。它和 TODO 是同一个组件，差别只在 DSL。
...
area 顶层的 `when` 是这块列表的总过滤，各 `section.when` 在其内再细分（同样是 AND 收窄）。
...
每个 list/grid/week/month area、每个 section 的过滤，就是它**自己的 `when`**，互不影响。
...
下面是 7 个内置视图的**真实出厂 DSL**（取自 `src/builtin-views/*.json`），可直接当模板改。注意它们都**没有** tab 级 `filters`，状态等过滤一律落在 area / section 的 `when` 上。
...
**一个** list + 3 个 section（逾期 / 今天 / 未排期），area 级 `when` 取 `todo`：
...
3. **过滤只属于 area**：每个 area / section 的 `when` 就是它的全部过滤，没有 tab 级基础集、没有全局过滤状态。
