...
> 🖼️ **配图 TODO** —— 放弃区单独的接收态：把一张任务卡拖到「放弃区」上方时的高亮/悬停反馈。点位：此说明与下方示例代码之间。
...
> 🖼️ **配图 TODO** —— 构造一个 `type` 写错的 area，截「未知类型 + 原始 JSON」的兜底渲染。点位：本节正文下方。
...
> 🖼️ **配图 TODO** —— 当前截图是空态（vault 暂无已完成任务，只有顶部 summary 芯片）。造几条带 `actual`/`estimate` 的 done 任务后重拍，展示真实卡片 + 求和/比率数值。
...
图形过滤入口、「编辑 Query」直编 JSON、CLI 查询管理，三者共用同一份 DSL 与校验：在界面上点过滤器改的就是某个 area 的 `when`，和你在这份 JSON 里手写完全等价。

> 🖼️ **配图 TODO** —— 并排展示「图形过滤弹窗」与对应的 `when` JSON 片段，标出二者一一对应。点位：本节正文下方。

> **TODO（待写）**：
> - CLI 侧如何读写同一份 preset（`obsidian task-center:*` 查询命令与本 DSL 的映射）。
> - 图形编辑落进 tab draft 的时机、与「保存 / 另存为」的关系。
