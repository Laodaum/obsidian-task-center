...
参考 Office Ribbon、Notion、Figma、Linear、VS Code 处理"命令太多"的共同答案：**不是把杂物折叠/隐藏到一个 `⋯` 抽屉，而是按命令作用的对象把它归位、并按上下文呈现。** 一个混装了不同对象动作的 `⋯` 抽屉（"杂物抽屉"）是反模式——藏起来不等于语义变好。

规则：

1. **按对象归位**：命令长在它**作用的对象**的"家"里。
   - 作用于"当前 Query"的（编辑、另存、更新、丢弃）→ 留在 Query 工具栏 / 编辑 Query 面板。
   - 作用于"Tab 集合"的（管理、新建、恢复预设）→ 放在 **Tab Strip**（tab 的家），不放进 Query 工具栏。
   - 作用于"整个应用"的（设置）→ 应用级 chrome（顶部独立齿轮 / Obsidian 设置 / 命令面板），不与 Query 动作平级。
   - 作用于"单个任务/单个 tab"的 → 长在那个对象上（卡片菜单 / 行级 kebab）。
2. **上下文呈现**（Office 上下文标签 / Figma 属性面板）：只在对象相关时显示。例如保存类动作（更新 / 另存 / 丢弃）**只在有未保存改动时出现**，干净状态不显示。
3. **一个清晰主操作**（Things 的 `+`）：高频创建动作始终显眼。
4. **可发现性兜底**：低频 / 全局命令交给固定可预测的家（设置、Obsidian 命令面板），不占工具栏。
5. **重复结构用行 + kebab**：列表每项压成一行（名称 + 徽标 + 右侧 `⋮`），点击行 = 该项主操作，低频进 `⋮`。
6. **禁止杂物抽屉**：不要把分属不同对象的动作塞进同一个 `⋯`。如果你想加一个 `⋯`，先问"这里面的项是不是同一个对象的"——不是就拆开归位。

> e2e 选择器契约：被 e2e 依赖的动作（带 `data-action`）保持真实 `<button data-action>`，可见或上下文出现，不要塞进 Obsidian `Menu`（Menu item 不在受控 DOM 里）。无 e2e 依赖的行级 kebab 用原生 `Menu` 即可。
>
> 具体哪个动作归到哪个对象写在 `UX.md`；本文档只定原则。
