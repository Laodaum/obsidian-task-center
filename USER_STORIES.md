...
`US-109p6` Query 编辑面板的四块内容（Filters / View / Summary / DSL）必须收进**同一个带 Tab 的面板**，靠顶部一排 Tab 切换，而不是从上往下铺成一长条。用户进面板时一次只看一块，避免在小屏上长滚动找不到目标。四个 Tab 固定为：筛选（Filter）、统计（Summary）、视图（View）、DSL（完整 DSL 直编）。任何打开 Query 编辑的入口（工具栏「编辑 Query」、summary 上的铅笔、area header 上的过滤漏斗）都进同一个 Tab 面板，只是默认落点不同：

- 「编辑 Query」/ tab 菜单 / 工具栏摘要：默认落在筛选 Tab。
- summary 铅笔 / 「+ 添加指标」：默认落在统计 Tab。
- area header 的过滤漏斗：仍保留 area header 上就地编辑该 area `when` 的 popover（US-109w/x/y 不变）；但漏斗也提供一个进入统一面板的路径，进面板时默认落在筛选 Tab，编辑的是共享基础集 `preset.filters`（US-109z）。per-area 的 `when` 收窄和基础集 `preset.filters` 是两个不同对象，面板筛选 Tab 永远编辑后者，不取代前者。
