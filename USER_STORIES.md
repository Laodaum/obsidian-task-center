...
`US-153` 在当前 view 里点 ✔ 把一条任务标记完成后，这张卡不立即从当前列表消失，而是当场变成完成显示态（US-152 的"变色 + ✔"），停留在原位，直到用户下次重新进入 / 重新加载这个 view 才按正常过滤规则消失。

- 场景：用户在今日 / TODO / 未排期等"只看 todo"的 list view 里点 ✔ 完成一条任务。这些 view 的过滤会排除已完成任务，按旧行为这张卡会立刻被动画移除、彻底消失。用户常常在刚点完的一瞬间想"哦点错了"或"再看一眼"，却已经抓不到它了。
- 价值：让"刚完成"的卡片在本次浏览会话里仍然停留、可交互（再点一次取消完成、查看、滑动），把"完成"从一个不可逆的瞬间动作，变成一个可当场反悔的状态切换。
- 成功结果：点 ✔ 后卡片原地变完成态、不被移除；用户可以在它身上继续操作；只有当用户**重新进入这个 view**（切到别的 tab 再切回来、整页重新加载、外部数据变化触发的整表刷新）时，这些刚完成的卡才按该 view 的过滤规则消失。再次点 ✔（取消完成）会让任务恢复正常态。
- 边界：这个"暂时豁免过滤"只对**因用户在本 view 里点 ✔ 而刚完成**的任务生效，且只在本次 view 会话内有效；它不改变任何持久过滤语义、不影响 CLI / summary / badge 计数对完成任务的归类，也不影响放弃（✕）等其它状态的现有移除行为。
