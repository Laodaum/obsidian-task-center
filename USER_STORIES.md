...
`US-109q` Tab 过多时，多余 tab 收进“更多”。它们仍是一等 tab，排序、badge、默认 tab、快捷键顺序都按用户配置工作。“更多”首要价值是快速切换到溢出 tab：点一下就近展开溢出列表、点一行立刻切过去，不打断当前上下文。桌面在“更多”按钮就地展开下拉浮层（不全屏 / 不底部 modal）；移动端窄屏改用底部 sheet。每个 tab 的管理动作（重命名 / 复制 / 设默认 / 移动 / 隐藏 / 删除）收进该行的 `⋮` 菜单；溢出 tab 的重排入口在「管理 Tabs」面板，不要求在窄下拉里支持拖拽。
