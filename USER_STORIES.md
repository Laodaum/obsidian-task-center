...
`US-100` V1 提供五种 view。View 回答的是“同一批任务应该怎样被用户看见和操作”，不是回答“任务属于什么业务分类”。
...
| `horizon` 视界 | 4 列：今天 / 本周 / 下周 / 本月 | “今天、本周、下周、本月分别要做什么？” | 做精力分配和周计划的人 |
...
`US-103b` Horizon 视界视图把当前 query 命中的任务按时间远近放入四个固定列：今天、本周（不含今天）、下周、本月（不含本周/下周）。

它长这样：

- 桌面端是 4 个日期列：今天、本周、下周、本月。
- 移动端是 4 个可折叠行，布局与 week view 的 mobile 折叠模式一致。
- 每一列/行顶部分别显示对应的任务数与估时合计。
- 今天列默认高亮。
- 任务卡放在对应的时间桶下。
- 没有有效 `⏳` 的任务不进入日期列（可落入未排期 tray）。

它服务的用户问题：

- 用户想知道"今天、本周、下周、本月分别要做什么"。
- 用户想在同一屏上看到四个时间桶的负载分布，不用切换周/月视图。
- 用户想快速分辨哪些任务急需今天处理，哪些可以放到后面。
- 用户做周计划 / 精力分配时需要一个综合的 upcoming 全景。
...
| 本月 | `effectiveScheduled >= monthStart` 且 `effectiveScheduled <= monthEnd` 且不在本周/下周范围内 |

其中本周/下周边界遵循用户设置的"一周第一天"（US-112）。

Horizon view 提供的主要操作：
...
- 它不是"本周 + 本月"的组合查询；它是同一个 query 的结果按时间桶投影。
- 不属于任何桶的任务（无 `⏳` 或排期超出本月）不落入日期区，可以进入未排期 tray。
- 分桶规则是固定逻辑，不是用户可配置的 filter/section。
