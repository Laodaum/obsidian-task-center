...
`US-720d` 当一个 view 的所有内容 area 都是（无落区的）list/grid 且全部为空时，展示**一个居中的 view 级空状态**（替代各 area 各自的小空状态）——今日三组皆空、四象限 / TODO 全空都受益，而不是堆叠多个空块或留白页。这是通用机制，不是今日专属（删 sections、今日改 col[ list×3 ] 后，用它恢复「今天没有可执行任务」那种统一居中空状态）。空状态用 `bt-view-empty`（`data-empty-state="view"`，并保留 `data-today-empty` 钩子）。
