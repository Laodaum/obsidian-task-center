...
**面板按 area 上下文打开（US-109p9）**：`openQueryControlsSheet({ areaIndex?, initialTab? })` 把 `areaIndex` 存到 `queryEditorAreaIndex`。`renderFiltersTab` 用 `areaWhenByIndex(queryEditorAreaIndex)` 取该 area 的 `when`：list/grid 返回对象（渲染 "本视图过滤" `data-filter-section="area"` 小节，复用 `renderAreaFilterControls` + `setAreaWhen`），week/month/drop 返回 null（跳过该小节）；其后始终渲染 "基础集" `data-filter-section="base"` 小节（`renderSavedViewsFilterControls` 编辑 `preset.filters`）。`renderViewTab` 在有 `queryEditorAreaIndex` 时先渲染 area 标题输入框（`areaTitleByIndex` / `setAreaTitle`，写该 area 的 `title`，空则清除回退到内置本地化标题）。这是 UI 收敛，不改数据模型——`when` 仍只在 list/grid（`ListLikeFields`），week/month 不新增 `when`。
