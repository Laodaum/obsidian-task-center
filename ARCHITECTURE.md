...
**Query 编辑器是 Tab 化面板（US-109p6）**：`openQueryControlsSheet(initialTab?)` 在 `BottomSheet` 里渲染一个带 Tab 的面板（桌面、移动同一套 DOM，只是外层 sheet class 不同）。`renderQueryControlsSheet` 顶部渲染 Tab 条（`data-query-editor-tabs`，每个按钮 `data-query-tab="filter|summary|view|dsl"`），下面只渲染当前 Tab 的内容。四块内容拆成 `renderFiltersTab / renderSummaryTab / renderViewTab / renderDslTab` 私有方法，各自复用原有渲染逻辑（filter controls、summary 草稿 helper、view 类型按钮、DSL stringify/parse）。当前选中的 Tab 存在 `queryEditorTab` 实例字段上，跨重渲染保留；切 Tab 不重置 tab draft、不丢 DSL 校验错误位置。各入口传入默认 Tab：编辑 Query → `filter`，summary 铅笔 → `summary`。“保存与管理”动作区在 Tab 内容下方常驻，不属于任何单个 Tab。

**筛选 Tab 的两层结构（US-109p7 / US-109p8）**：`renderFiltersTab` 渲染两个小节：

- "本视图过滤" `data-filter-section="area"`：编辑**承载 summary 摘要栏那个 area** 的 `when`。复用 `renderAreaFilterControls`（从原 `renderAreaFilter` popover 抽出的纯控件渲染），调用 `setAreaWhen(areaIndex, …)` / `areaTags` / `statusFilterOptions` / `timeFilterOptions` / `collectTagOptions`，与 DSL 编辑同一份 area `when`。目标 area 的 DFS index 在渲染视图布局时被记到 `summaryAreaIndex`（即 `summaryPlaced` 落点那个 area）。若本次没有摘要栏 area（`summaryAreaIndex === null`），跳过本小节。
- "基础集" `data-filter-section="base"`：`renderSavedViewsFilterControls`，编辑 tab 级共享基础集 `preset.filters`。

**摘要栏 area 不挂就地漏斗（US-109p7）**：`renderListArea` 里 `isFirstContent` 的 area（即 `renderSummaryInto` 落点）不再调用 `renderAreaFilter`；它的 `when` 改由筛选 Tab 的 area 小节编辑。其它非摘要栏的可过滤 area 仍调用 `renderAreaFilter` 就地展开 popover（`filterPopoverArea` 状态、`data-area-filter` 钩子不变）。判定用 `isFirstContent`（= `filterable && !summaryPlaced`）：summary 落点那一个 area 走面板，其余走就地 popover。
