...
- `updateAreaAt(layout, areaIndex, patch)`：改第 N 个 area 叶子的字段（`when` / `title` / `type` / `sections` / `weight`）。**改 `type` 只换这一个叶子，保留 title / weight，不重建树**——修掉旧 `buildLayoutForAreaType` 重建整棵树的 bug。
