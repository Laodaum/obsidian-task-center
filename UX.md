...
桌面工具栏当前是两行：第一行是时间范围选择器（`◀ 第21周 ▶ 05-18→05-24` / `◀ 今天 ▶ 2026-05`）+ 搜索框，第二行是过滤 chip（标签 / 排期 / 更多时间 / 待办）。这是错位的：

- **时间范围选择器属于视图本身，不属于全局工具栏。** 它只在周 / 月这类有时间轴的视图里存在；列表 / 矩阵视图没有"上一周 / 下一月"的概念。放在外层工具栏，就是让全局工具栏耦合某个具体视图的内部状态。
- 正确归属：范围导航跟着**组件（area）走**——周组件渲染并拥有自己的 `◀ 周 ▶`，月组件渲染并拥有自己的 `◀ 月 ▶`。视图切到列表 / 矩阵时它随组件一起消失，外层不需要判空。

由此，桌面工具栏收敛为**单行** = 搜索框 + 过滤 chip（+ 与时间无关的通用操作），把原来的两行并成一行，省下一整行垂直空间（看板高度本就紧张）。范围导航条的视觉（chip / 箭头按钮 / 当前范围文字）遵循 `DESIGN.md` 的 token 与状态规范。

移动端的两行规则（日期导航独占一行）见 §6.2，仍然成立——这里收敛的是桌面端。

**动作区收敛（主操作 + 溢出）**：工具栏右侧的动作不再平铺成一排等权重按钮（另存为新 tab / 编辑 Query DSL / 管理 Tabs / + 新建 / ⚙️），改为：

```text
[更新][丢弃]（仅有未保存改动时）   [+ 新建]   [⋯]
```

- `+ 新建`（Quick Add 任务）是高频主操作，始终可见。
- 有未保存改动时，`更新` / `丢弃` 作为主操作可见（见 §2.4）。
- 其余低频项收进 `⋯` 溢出：另存为新 tab、编辑 Query DSL、管理 Tabs、设置。
- 溢出项仍是带 `data-action` 的真实按钮（自定义 popover），保持 e2e 选择器契约（`save-current-view` / `edit-current-view-dsl` / `manage-query-tabs`），不要用 Obsidian `Menu` 替换。
- 遵循 `DESIGN.md` §5.0。
