...
> **area head 是「区段标题」，要和内容卡片视觉分层（用户反馈）**：area head（WEEK / 未排期 / 象限名…）必须一眼读作区段标题，不能和下方的内容卡片（周视图日卡片 `bt-week-col`、任务卡）长一个样。规则：head **扁平**（不占卡片填充色），底部一条细分隔线，**吸顶**（`position: sticky`，滚动 area 内容时标题保持可见并压在内容之上）；内容卡片才是带填充色 + 圆角的抬升块。窄屏（移动布局）尤其重要——此前 head 和日卡片都用 `--background-secondary` 圆角卡片，完全分不开。空的未排期 tray 的占位（「没有未排期任务」）是轻量左对齐 faint 提示，不是抢重心的大块居中文案。
...
- **area head 标题恒非空**（list / grid / week / month 同一条路径，不写 per-type 例外）：优先显式（本地化）标题；视图首个内容 area 无显式标题时借用视图名（所以 week / month 视图的首个 area 收起时显示「本周」「本月」），再无则退回 area 类型名兜底。
- **head 是可点折叠条**：移动端 area head 给可见底色 + 圆角 + 足够触控高度（约 40px）+ hover/按压态，右侧 ▸/▾ 标识放大（约 14px、`text-muted`，hover 提亮）不再细弱，「收起（▸）vs 展开（▾）」状态一望可知。仅 `[data-mobile-layout="true"]` 生效，桌面不变。
...
- 任务总览（今日 / 逾期）由状态栏提供（US-106）。（US-512）
