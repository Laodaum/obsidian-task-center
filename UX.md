...
### 3.2 编辑 Query：两个独立面板（Tab 面板 / Area 面板）

编辑分两个互不混淆的对象，由**入口决定打开哪个面板**（US-109p10）。规则一句话：**对所有 area 生效的 → Tab 面板；只作用于一块内容的 → Area 面板。**

#### 3.2.1 Tab 面板（编辑整个 Tab）

入口：工具栏「编辑 Query」、tab 菜单「编辑 Query」、工具栏当前 Query 摘要。标题用 tab 名（如 `编辑 Query · Today`）。内容只有 tab 级对象，自上而下：
...
外观
  标题:  [逾期                    ]
  类型:  (列表) 网格 周 月 放弃区
  分组:  …（list / grid 才有 sections）
─────────────────────────────────────────────────────
ℹ︎ 基础集（状态:TODO #work）也叠加在本区   〔去 Tab 改基础集〉
```

- **本区过滤**（`data-filter-section="area"`）：该 area 的 `when`（搜索 / 状态 / 排期 / 标签），即原 area 漏斗 popover 的那套控件，写进 tab draft、DSL 可改（US-109x / US-109y）。week / month 无 `when`，此时不显示本区过滤小 tab，只显示外观。
- **外观**：标题（`data-area-title-input`）+ 类型 + list/grid 的分组 sections。类型是 **area 级**，只换这一块，不重建 layout（US-109p11）。
- 底部一行**只读**提示说明"基础集也叠加在本区"，并给跳去 Tab 面板改基础集的链接——把"全局层"降级成提示，不再和本区过滤并列成两坨（US-109p10）。
- Area 面板不放基础集 chip 墙、不放布局树、不放 DSL（那些是 tab 级）。
...
- 复杂嵌套对窄屏不友好时，移动端「布局」小节退化为**单层区域列表 + 增删排序**，深层嵌套引导去 DSL；桌面才给完整树。
...
> **US-109p9 / US-109p10 取代上面"就地漏斗 popover"的形态**：不再每个 area 各画一个就地展开的漏斗 popover、也不再有"摘要栏 area 例外"的分叉。改为：**每个内容 area（list / grid / week / month / 四象限 / 未排期 tray）共用同一套 area head——标题 + 一个统一「编辑」按钮（`bt-area-edit`，图标 `sliders-horizontal`，激活时在按钮上显示该 area 的 `when` 摘要）。点它打开 §3.2.2 的「Area 面板」，「本区过滤」改这个 area 自己的 `when`（week/month 无 `when` 则不显示本区过滤小 tab），「外观」改并保存该 area 的标题与类型。** 上面关于"作用域是这一个 area / 编辑该 area 的 `when` / 空状态归因"的语义不变，只是入口打开的是 Area 面板（不是 Tab 面板）。drop / week / month 与 list/grid 同一套 head；纯 drop 放弃区无 query，不给编辑入口。
