...
#### 8.1.2 刚完成卡片的停留与延迟消失（US-153）

在一个会按状态过滤掉已完成任务的 view（今日 / TODO / 未排期等）里点 ✔ 完成任务时：

- 卡片**不**播放"淡出移除"动画、**不**立即从列表消失；它当场原地变成完成显示态（§8.1.1 的"变色 + ✔"），停在原位。
- 这张卡仍可交互：再次点 ✔ 取消完成（任务恢复正常态并继续停留）、单击查看原文、移动端滑动等。
- 这些"刚完成"的卡片只在用户**重新进入该 view**时才按正常过滤规则消失。重新进入包括：切到别的 tab 再切回、整页重新加载（onOpen）、外部数据变化触发的整表刷新（cache changed → scheduleRefresh）。
- 取消完成（再次点 ✔ 变回 todo）后，卡片回到普通态，不再带完成态样式。
- 该"停留豁免"只对本 view 会话内、由用户点 ✔ 而刚完成的任务生效；放弃（✕）等其它状态仍按原有"淡出移除"行为处理（US-127）。
- e2e / 渲染契约：刚完成且处于豁免停留态的卡片仍带 `done` class 与 `bt-check-done`，并额外打 `data-just-completed="true"` 标记，便于断言"它还在列表里"。
