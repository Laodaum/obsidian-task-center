...
- **例外：承载 summary 摘要栏的那个 area 不挂就地漏斗**（US-109p7）。它 header 右侧只渲染 summary chips + 铅笔；它的 `when` 改到统一面板「筛选」Tab 的"本视图过滤"小节里编辑（§3.2）。这样避免摘要栏那行出现"铅笔 + 漏斗"并排的重复入口。**只有这一个 area 例外**——多 area tab 里其它没有摘要栏的 area（四象限另外三格、周视图未排期 tray 等）仍各自就地渲染漏斗 popover，行为不变。
