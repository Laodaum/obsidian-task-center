...
- 多选默认 AND 语义（「含全部」）；选中 ≥2 标签时出现「含全部 / 含任一」段切换（`bt-area-tag-mode`，按钮 `bt-area-tag-mode-btn[data-tag-mode="and|or"]`）。它出现在两处**互斥**位置：标签 popover **打开**时在 popover 内部顶部（搜索框上方），让用户选标签的当下就能看见、切换；popover **关闭**时在标签区头部右侧，方便已选好后快速调整。单标签时不显示（AND/OR 无差异）。（US-109d2）
- 点击整行切换选中态。
