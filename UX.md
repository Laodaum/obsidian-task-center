...
3. **View 只改变呈现，不改变集合**：`list / grid / week / month / matrix / horizon` 是布局和操作方式，不是业务分类。（US-100 / US-109k / US-103b）
...
### 6.6 Grid / Area Layout

DSL 用 area 树描述布局，每个 area 是一个命名空间，携带自己的 `title`、`type` 和 `when`：
...
- **head 是「标签页质感」的单开折叠条**（移动端）：圆角、贴底色的可点条（约 38px 触控高度），去掉旧版灰描边下划线 + 直角的廉价感。折叠（未选中）态克制——背景透明、标题 `--text-muted`、▸ 用 `--text-faint`，hover/按压才浮出 `--background-modifier-hover`；当前展开（选中）那条清晰高亮——半透明软 accent 背景（`color-mix(--interactive-accent 8%, --background-modifier-hover)`）+ 标题转 `--text-normal` + ▾ 着 accent，「当前打开哪个 area」一望可知。选中高亮**只用背景高亮**，**禁止左侧 accent 竖条 / `box-shadow: inset` spine / 实心边框 / 四周环**（左 spine 被 `local.no-inset-left-border` 明令禁止；实心环会与卡片选中态、「+新建」按钮撞色）。全走主题变量，仅 `[data-mobile-layout="true"]` 生效，桌面不变。（US-511a）
...
- **展开内容比 area 内缩一档**：手风琴容器自身贴边，但展开 area 的 body（`.bt-list-view` / `.bt-week` / 月格容器）左右各内缩 8px，使 entry card / 周·月内容读作「嵌在 area 里、比 area 小一圈」，靠页面底色的留白槽表达层级——不再 edge-to-edge 贴死容器边（用户反馈：贴边像撞在一起）。卡片自身内边距不变。（US-511a）
- 桌面端不变，多 area 仍同时铺开。
...
- **accent 预算**：蓝色 `--interactive-accent` 满血填充只留给唯一主 CTA「新建」；所有**选中态**（卡片选中、area 展开高亮）改用软背景填充，不再用 accent 蓝实心边框/环、也不用左侧 accent 竖条（`local.no-inset-left-border`），避免「满屏都是蓝」。`+新建` 与筛选按钮共享居中基线、统一触控高度，不悬浮错位。
