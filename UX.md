...
- 点击整行在三态间循环：**不管 → 包含 → 排除 → 不管**（US-109d3）。包含行左侧 `✓`（accent）、排除行左侧 `⊘` 且标签文字带删除线/弱化、不管行无标记；行用 `data-area-tag-state="include|exclude"` 标记当前态。
- 触发器（trigger）摘要分别概括包含与排除，如「含 #a +1 · 排除 #c」。
- 包含组默认 AND 语义（「含全部」）；包含标签 ≥2 时出现「含全部 / 含任一」段切换（`bt-area-tag-mode`，按钮 `bt-area-tag-mode-btn[data-tag-mode="and|or"]`），出现在两处**互斥**位置：标签 popover **打开**时在 popover 内部顶部（搜索框上方），让用户选标签的当下就能看见、切换；popover **关闭**且 ≥2 包含标签时在标签区头部右侧，方便已选好后快速调整。单个包含标签时不显示（AND/OR 无差异）。排除组恒为「都不含」，无与/或之分。（US-109d2 / US-109d3）
