...
- **时间范围选择器属于视图本身，不属于全局工具栏。** 它只在周 / 月这类有时间轴的视图里存在；列表 / 网格视图没有"上一周 / 下一月"的概念。放在外层工具栏，就是让全局工具栏耦合某个具体视图的内部状态。
- 正确归属：范围导航跟着**组件（area）走**——周组件渲染并拥有自己的 `◀ 周 ▶`，月组件渲染并拥有自己的 `◀ 月 ▶`。视图切到列表 / 网格时它随组件一起消失，外层不需要判空。
