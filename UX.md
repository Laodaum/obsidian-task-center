...
1. **筛选（Filter）**：见下"筛选 Tab 的两层结构"。
2. **统计（Summary）**：count、sum、ratio、top-N、group-by。
3. **视图（View）**：类型、section、排序、未排期 tray、布局嵌套。
4. **DSL**：完整 Query DSL 直编（高级模式）。

**筛选 Tab 的两层结构（产品决策，US-109p7 / US-109p8）**：面板「筛选」Tab 自上而下分两个有清晰标题的小节，分别编辑两层过滤：

1. **本视图过滤**（`data-filter-section="area"`）：编辑**承载 summary 摘要栏那个 area** 的 `when`（搜索 / 状态 / 排期 / 标签）。这套控件即原来 area header 上漏斗 popover 的内容，搬进面板里，复用 area `when` 的同一份数据（写进 tab draft，DSL 可改）。
2. **基础集**（`data-filter-section="base"`）：编辑 tab 级**共享基础集 `preset.filters`**，对该 tab 所有 area 生效（让今日 / TODO 等内置单 area tab 不破，US-109z）。

两小节是不同对象，互不取代。把它们放进同一个 Tab，是因为摘要栏 area 的漏斗已被去掉（US-109p7），它的 `when` 需要一个明确的图形入口；和基础集放在一起，用户在一个 Tab 里就能完成"本视图收窄 + tab 基础集"两层编辑。当前 tab 若没有承载摘要栏的可过滤 area，则只显示"基础集"小节。

**面板入口与默认 Tab**：

- 工具栏「编辑 Query」、tab 菜单「编辑 Query」、工具栏当前 Query 摘要：打开面板，默认落在筛选 Tab。
- summary 上的铅笔 / 「+ 添加指标」：打开同一面板，默认落在统计 Tab；用户点「筛选」Tab 即可切到上面两层过滤。摘要栏 area 现在**只有铅笔一个入口**，不再并排漏斗（US-109p7）。
...
**稳定选择器契约**：面板根保留 `data-saved-views` / `data-query-editor`；Tab 条为 `data-query-editor-tabs`，每个 Tab 按钮带 `data-query-tab="filter|summary|view|dsl"`；DSL 输入框保留 `data-query-dsl-input`，统计字段保留 `data-summary-field`，summary 行保留 `data-summary-metric`。筛选 Tab 的两小节分别带 `data-filter-section="area"`（本视图过滤）与 `data-filter-section="base"`（基础集）。切换 Tab 不改变这些钩子的取值。
...
> **US-109p9 取代上面"就地漏斗 popover"的形态**：不再每个 area 各画一个就地展开的漏斗 popover、也不再有"摘要栏 area 例外"的分叉。改为：**每个内容 area（list / grid / week / month / 四象限 / 未排期 tray）共用同一套 area head——标题 + 一个统一「编辑」按钮（`bt-area-edit`，图标 `sliders-horizontal`，激活时在按钮上显示该 area 的 `when` 摘要）。点它打开统一 Tab 面板，「筛选」Tab 编辑这个 area 自己的 `when`（week/month 无 `when` 则只显示基础集），「视图」Tab 改并保存该 area 标题。** 上面关于"作用域是这一个 area / 编辑该 area 的 `when` / 空状态归因"的语义不变，只是入口从就地 popover 换成统一面板。drop / week / month 与 list/grid 同一套 head，不再各写一套。
